...
qx.Class.define("svg.behavior.Draggable",
{
   ...
   *   Initial y offset.
   */
  construct : function(svgElement, x, y, downCallback, moveCallback, upCallback)
  {
    this.base(arguments);
    ...
    this.__downCallback = downCallback?downCallback:null;
    this.__upCallback = upCallback?upCallback:null;
    this.__moveCallback = moveCallback?moveCallback:null;
  ...
  members :
  {
    ...
    __onMouseDown : function(e)
    {
   	  ...
   	  // if (!this.__element.getModel().isDrag) {
      //   return;
      // };

      if (this.__downCallback) {
        this.__downCallback(e);
      };
    ...
    {
      if(this.__moveCallback){
      	this.__moveCallback(e);
      };
      e.stopPropagation();
      var curMousePos = this.__convert(this.__element, e.getDocumentLeft(), e.getDocumentTop());
